...
/* 官方 SettingsRoot.module.css 类名（运行时由 dsh-client-ui-settings-general 注入，本机 rc.2 hash；
 * 完全沿用官方名字与样式——不自写、不自创。 */
const C = {
...
const OFFICIAL_CSS = [
  '.VOzbGW_trigger{box-sizing:border-box;cursor:pointer;width:calc(100% + 4px);height:42px;color:var(--dsw-alias-label-primary);background:0 0;border:none;border-radius:12px;flex:none;align-items:center;gap:8px;margin:4px -2px;padding:0 10px 0 8px;font-family:inherit;font-size:14px;line-height:22px;display:flex;overflow:hidden}',
  '.VOzbGW_trigger:hover{background:var(--dsw-alias-interactive-bg-hover)}',
  '.VOzbGW_trigger.VOzbGW_rail{border-radius:50%;justify-content:center;gap:0;width:36px;height:36px;margin:8px 0 10px;padding:0}',
...
const EXTRA_CSS = [
  ...
  '}',
  'html[data-dsh-skin="matrix"] [role="dialog"]  .VOzbGW_options {',
...
const NATIVE = ['general', 'models', 'plugins', 'agent-presets']
...
export function SettingsShell(props: ShellProps) {
  ...
  const nativePresent: NavRowLike[] = NATIVE.map((id) => navRows.find((r) => r.id === id)).filter(
  ...
  useEffect(() => {
    if (open && target === null) {
      setTarget(mode === 'fold' ? { kind: 'section', id: NATIVE[0] } : { kind: 'mega' })
    }
  }, [open, target, mode])
  useEffect(() => {
    if (!open) return
  ...
  const targetAt = (x: number, y: number): OverTarget => {
    const root = navListRef.current
      ...
      if (!closed['$fold']) {
        ...
        for (const g of groups) {
          ...
          rowsNow.forEach(({ it, rank }, pos) => {
            if (slotO === pos && pos <= N) sub.push(foldStrip(g.id, slotO, 'gz-slot'))
            sub.push(
              <DragNavCell
                key={'s' + it.id}
                kind="body"
          ...
          if (slotO === N && slotO >= rowsNow.length) sub.push(foldStrip(g.id, slotO, 'gz-slot'))
        }
