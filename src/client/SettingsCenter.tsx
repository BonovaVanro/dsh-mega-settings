...
function CardShell(props: {
  ...
  /** 版本 fetch 是否已结束（加载中 tag 留空，不闪「未知」） */
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="mgs-card-tag">
          {props.versionOf?.(props.it.entry) ?? (!props.versionsDone ? '' : tr('settings.unknown'))}
        </span>
      </div>
...
function GroupSection(props: {
  ...
  if (props.items.length === 0) {
    ...
    // 组内仅剩被拖项：视作空组——整行宽接收槽 + 被拖卡仍挂载（fixed 跟手，不能卸载）
    const effEmpty =
      props.dragId !== null &&
      props.dragId !== undefined &&
      props.items.length === 1 &&
      props.items[0].id === props.dragId
    if (effEmpty) {
      cells.push(
        <div
          key="rec-eff-empty"
    ...
    props.items.forEach((it, idx) => {
      if (!effEmpty && props.frameP !== null && props.frameP !== undefined && props.frameP === idx) {
      ...
      cells.push(
        <CardShell
          ...
          versionsDone={props.versionsDone}
          ops
    ...
    if (!effEmpty && props.frameP !== null && props.frameP !== undefined && props.frameP >= props.items.length) {
...
function GroupDragPane(props: {
  ...
  const attach = (): void => {
    ...
    const move = (e: PointerEvent): void => {
      ...
      const left = Math.max(8, Math.min(e.clientX - d.grabX, w - d.width - 8)) - d.ox
      const top = Math.max(8, e.clientY - d.grabY) - d.oy
      applyFly(left, top)
      // 悬停判定较重，仍每帧合并一次
      d.px = e.clientX
      d.py = e.clientY
  ...
  return (
    <div
      ...
      onPointerMove={(e) => {
        ...
        const w = typeof window !== 'undefined' ? window.innerWidth : 1200
        applyFly(
          Math.max(8, Math.min(e.clientX - d.grabX, w - d.width - 8)) - o0.x,
          Math.max(8, e.clientY - d.grabY) - o0.y,
        )
        setFly(true)
...
export function SettingsCenter(props: SettingsCenterProps) {
  ...
  const versionOf = (entry: SectionEntry): string | undefined => {
    // 优先同步：条目 inject 面带 version（如 mega 成员）——无 fetch 延迟，首帧即有
    const syncV = memberVersion(entry as MemberEntry)
    if (syncV) return syncV
  ...
  const handleCardHover = (x: number, y: number): void => {
    ...
    // 只做一次 elementFromPoint，结果同时供落点判定与边缘滚动定位
    const el = document.elementFromPoint(x, y)
    updateAutoScroll(x, y, el)
  ...
  /** 依指针位置定位最近可滚动容器并进入/退出边缘滚动（hit 复用 handleCardHover 的 elementFromPoint 结果） */
  const updateAutoScroll = (x: number, y: number, hit?: Element | null): void => {
      ...
      <section className="mgs-root" data-mega-settings-root>
        <header className="mgs-header">
          <div className="mgs-header-left">
            <HostBadge label={tr('settings.center')} />
            <span className="mgs-header-divider">&gt;</span>
            <Badge name={label} tag={ver ?? (!done ? '' : tr('settings.managed'))} />
          </div>
          <button className="mgs-back" type="button" onClick={back}>← {tr('settings.back')}</button>
        </header>
    ...
    <>
      ...
      {(() => {
        ...
        const dragPane = (g: SettingsGroup, gi: number): ReactNode => {
          ...
          // 组内仅剩被拖项 → 视作空组（整行宽接收槽）
          const effEmpty = dragId !== null && gItems.length === 1 && gItems[0].id === dragId
          return (
            <GroupDragPane
              key={'gp-' + g.id}
              gi={gi}
            ...
            >
              <GroupSection
                ...
                emptyRec={dragId !== null && (isEmpty || effEmpty)}
                recOn={dropP !== null && dropP.key === g.id && (isEmpty || effEmpty)}
                dragId={dragId}
                {...cardViewProps}
                gridKey={g.id}
