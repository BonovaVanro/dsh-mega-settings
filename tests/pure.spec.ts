...
describe('v2: 分组运算', () => {
  ...
  it('createGroup：成功与失败（数组不变）', () => {
    const r1 = createGroup([], 'g1', '导航')
    expect(r1.error).toBeNull()
    expect(r1.groups).toEqual([g('g1', '导航')])
    const r2 = createGroup([g('a', '导航')], 'g2', '导航')
  ...
  it('moveItem：跨组移动/移入未分组/组不存在', () => {
    const groups = [g('a', 'A', ['x', 'y']), g('b', 'B', ['z'])]
    const m1 = moveItem(groups, 'y', 'b')!
    expect(m1[0].itemIds).toEqual(['x'])
    expect(m1[1].itemIds).toEqual(['z', 'y'])
    const m2 = moveItem(m1, 'y', null)!
    expect(m2[1].itemIds).toEqual(['z'])
    expect(moveItem(groups, 'x', 'nope')).toBeNull()
    // 组内已存在项不重复（先移除后插入语义）
    const m3 = moveItem(groups, 'x', 'a')!
    expect(m3[0].itemIds).toEqual(['y', 'x'])
  })
  it('reorderItem / reorderGroups', () => {
    const groups = [g('a', 'A', ['x', 'y', 'z']), g('b', 'B')]
    expect(reorderItem(groups, 'a', 0, 2)[0].itemIds).toEqual(['y', 'z', 'x'])
    expect(reorderItem(groups, 'a', 0, 9)).toEqual(groups) // 越界保持
